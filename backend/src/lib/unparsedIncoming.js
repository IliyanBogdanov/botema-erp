 ...
 *
 * Moved from a local Windows-scheduled script to a Railway cron job on
 * 2026-07-21 — Google Workspace only issues OAuth tokens to requests from
 * approved network origins, so the local machine can no longer refresh
 * GOOGLE_REFRESH_TOKEN; the Railway service can.
 */
const { Prisma } = require('@prisma/client');
const { google } = require('googleapis');
const { parseDocumentWithAI, downloadDriveFile } = require('./aiParser');
...
const sleep = () => new Promise(r => setTimeout(r, THROTTLE_MS));
...
async function runUnparsedIncoming(prisma, { apply = false, limit = Infinity, log = console.log } = {}) {
  log(`=== Unparsed incoming → BizDocuments ${apply ? '(APPLY)' : '(DRY RUN)'} ===`);
  ...
  log(`Без BizDocument: ${sfs.length} от ${all.length} PDF-а`);
  ...
  for (const sf of sfs) {
    ...
    try {
      if (SKIP_FILE.test(sf.filename)) {
        ...
        log(`SKIP-FILE ${tag} (очевидно не е фактура)`);
        continue;
      }
      ...
      if (ownSeries) { stats.skippedOwn++; log(`SKIP-OWN  ${tag} → наша фактура #${parsed.invoiceNo}`); await tagSkip('own outgoing'); await sleep(); continue; }
      if (!['INVOICE_IN', 'INVOICE'].includes(pType)) { stats.skippedNonInvoice++; log(`SKIP-TYPE ${tag} → ${pType} conf=${conf}`); await tagSkip(`parsed as ${pType}`); await sleep(); continue; }
      const amountEurEq = currency === 'BGN' ? pAmt / 1.95583 : currency === 'USD' ? pAmt / 1.14 : pAmt;
      if (!(amountEurEq > 0 && amountEurEq < 100000)) { stats.failed++; log(`INSANE    ${tag} → ${pAmt} ${currency}`); await sleep(); continue; }
      ...
      if (dup) { stats.skippedDup++; log(`SKIP-DUP  ${tag} → вече записан като ${dup.id.slice(0, 12)} #${dup.docNumber}`); await tagSkip(`duplicate of ${dup.id}`); await sleep(); continue; }
      ...
      log(`${promote ? 'REVIEWED ' : 'NEEDS-REV'} ${tag} → @${cp?.name || 'NO-CP'} ${currency} ${pAmt} #${parsed.invoiceNo || '?'} ${new Date(docDate).toISOString().slice(0, 10)} conf=${conf}`);
      ...
      if (apply) {
        await prisma.bizDocument.create({
          data: {
            ...
            confidence: dec(conf),
            notes: `Създаден от непарснат Drive файл: ${sf.folder || ''} ${sf.filename}`.slice(0, 500),
          },
        });
      }
    } catch (err) {
      stats.failed++;
      log(`FAIL      ${tag}: ${String(err.message).slice(0, 100)}`);
    }
    await sleep();
  }
  log(`=== DONE === ${JSON.stringify(stats)}`);

  // How many actionable files are still left (excludes obvious non-invoices,
  // which will never get a BizDocument even after re-scanning)
  ...
  log(`REMAINING_ACTIONABLE=${actionable}`);

  return { ...stats, remainingActionable: actionable };
}

module.exports = { runUnparsedIncoming };
